...
public class RootServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        String titleValue = Stream.of(cookies).filter(n -> "title".equals(n.getName()))
                .map(Cookie::getValue).findAny().orElse("default");
        req.setAttribute("titleValue", titleValue);

        String isbnValue = Stream.of(cookies).filter(n -> "isbn".equals(n.getName()))
                .map(Cookie::getValue).findAny().orElse("default");
